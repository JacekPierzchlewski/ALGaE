%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ALGaE_AdvOpt_GUI_editIdenNum1: THE "ALGaE" PACKAGE - GRAPHICAL USER INTERFACE,
%                                                      MODULE: MAIN GUI, OPTIONS
%                                            
%                                                      GUI SERVICES: 
%                                                           EDIT FIELD 'Identity number 1'
%
...
%                       Possible service:
%
%                           - 'Create' (edit text 'Identity number 1')
%
%                           - 'Initialize' (edit text 'Identity number 1')
%
%                           - 'Callback' (edit text 'Identity number 1')
%
%
% ------------------------------------------------------------------------
% Output:
...
function ALGaE_AdvOpt_GUI_editIdenNum1(hObject, handles, strServ)
 
    % Run the correct service
    switch strServ
                
        % Create the text edit box 'Identity number 1'
        case 'Create'
            edit_IdenNum1_create(hObject);
            
        % Initialize the text edit box 'Identity number 1'
        case 'Initialize'
            edit_IdenNum1_init(handles);

        % Callback the text edit box 'Identity number 1'
        case 'Callback'
            edit_IdenNum1_callback(hObject, handles);
    end
end


%% Service: create GUIedit_IdenNum1 object (list box 'Identity number 1')
function edit_IdenNum1_create(hObject)
...
%% Service: initialize GUIedit_IdenNum1 object (list box 'Identity number 1')
function edit_IdenNum1_init(handles)

    % Get the sScen scenario structure
    sScen = getappdata(handles.figure1,'sScen');
    
    % Initialize the correct value
    set(handles.GUIedit_IdenNum1,'String',num2str(sScen.N_ID1));
end


%% Service: callback of the GUIedit_IdenNum1 object (list box 'Identity number 1')
function edit_IdenNum1_callback(hObject, handles)
    ...
    % Get the minimum and maximum values of the Identity Number 1
    N_id1_min = sLTE_stand.N_id1_min;       % Minimum value
    N_id1_max = sLTE_stand.N_id1_max;       % Maximum value    
    ...
    if abs(iVal - iVal_old) > 0.000001
        ...
        set(hObject,'String',num2str(sScen.N_ID1));
    ...
    % Check if the value is correct
    if iVal > N_id1_max  || iVal < N_id1_min
