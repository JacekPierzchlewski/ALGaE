%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LTE_Professor_editFrstFreq:  THE "ALGaE" PACKAGE - GRAPHICAL USER INTERFACE,
%                                                    MODULE: LTE PROFESSOR
%
%                                                    SERVICE: EDIT FIELD 'First Subcarrier Frequency'
%                                                    (GUI object: 'Edit_frstFreq')
%
...
%                       Possible service:
%
%                           - 'Create' (Edit field 'First Subcarrier Frequency')
%
%                           - 'Callback' (Edit field 'First Subcarrier Frequency')
%
% ------------------------------------------------------------------------
% Output:
%
...
function LTE_Professor_editFrstFreq(hObject, handles, strServ)
    ...
    switch strServ
        ...
        % Create
        case 'Create'
            FrstFreq_create(hObject);

        % Callback
        case 'Callback'
            FrstFreq_callback(hObject,handles);
    end
end

%% Service: create 'Edit_frstFreq' object
function FrstFreq_create(hObject)
...
%% Service: edit 'Edit_frstFreq' object
function FrstFreq_callback(hObject,handles)
    ...
    fFrstSub = getappdata(handles.figure1,'fFrstSub');    
    ...
    % Minimum frequency
    if fFrstSub <= -N_scB/2*Delta_f;
        fFrstSub = -N_scB/2*Delta_f;
    end
        
    % Maximum frequency    
    if fFrstSub > (N_scB/2 - 23)*Delta_f
        fFrstSub = (N_scB/2- 23)*Delta_f;
    end
    ...
    if fFrstSub <= 0
        inxFrstSub = floor(fFrstSub/Delta_f + 1 + N_scB/2);        
    else
        inxFrstSub = ceil(fFrstSub/Delta_f + N_scB/2);        
    end
