%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LTE_Professor_editFrstSymbRS: THE "ALGaE" PACKAGE - GRAPHICAL USER INTERFACE,
%                                                     MODULE: LTE PROFESSOR
%
%                                                     SERVICE: EDIT FIELD 'First Symbol, Symbol in Radio Slot Index'
%                                                     (GUI object: 'Edit_frstSymbRS')
%                                                                                                                            
...
%                       Possible service:
%
%                           - 'Create' (Edit field 'First Symbol, Symbol in Radio Slot Index')
%
%                           - 'Callback' (Edit field 'First Symbol, Symbol in Radio Slot Index')
%
% ------------------------------------------------------------------------
% Output:
%
...
function LTE_Professor_editFrstSymbRS(hObject, handles, strServ)
    ...
    switch strServ
        ...
        % Create
        case 'Create'
            FrstSymbRS_create(hObject);

        % Callback
        case 'Callback'
            FrstSymbRS_callback(hObject,handles);
    end
end


%% Service: create 'Edit_frstSymbRS' object
function FrstSymbRS_create(hObject)
...
%% Service: edit 'Edit_frstSymbRS' object
function FrstSymbRS_callback(hObject,handles)
   ...
   % Get the string value from the edit text field
    ...
    if isnan(inxVal)
        ...
        strFrst = num2str(inxFrstSymbsRS);  % Last
        set(hObject,'String',strFrst);
        return;
    end
    
    % Check if the symbol in Radio Slot index value is in the boundaries
    ...
    inxFrstSymbsRS = round(inxVal);        
    if inxFrstSymbsRS < 0
        inxFrstSymbsRS = 0;
    end
    
    if inxFrstSymbsRS > (N_symbDL - 1)
        inxFrstSymbsRS = (N_symbDL - 1);
    end    
