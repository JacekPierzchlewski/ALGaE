%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% L1a_GUI_lb_band: THE "ALGaE" PACKAGE - GRAPHICAL USER INTERFACE,
%                              STACK:  DOWNLINK,
%                              MODULE: CHANNELS AND MODULATION
%
%                              GUI SERVICE: LIST BOX 'Bandwidth'
%                                                                                      
...
function L1a_GUI_lb_band(hObject,handles,strServ)
    ...
    switch strServ
        ...
        % Callback
        case 'Callback'
            lb_band_callback(hObject,handles);

    end
end


%% Service: create GUIlb_band object
function lb_band_create(hObject)
    ...
    % Add the ' MHz' string to channel bandwidth configuration names
    for inxCBnam=1:size(cvBW_channel,1)
        
        % Get the channel bandwidth name
        strCBNam = cvBW_channel{inxCBnam};
        
        % Add the ' MHz' string
        strCBNam = strcat(strCBNam,' MHz');
        
        % Store the channel bandwidth name
        cvBW_channel{inxCBnam} = strCBNam;
         
    end
    
    % Set the cell with cyclic prefix configuration names
    set(hObject,'String',cvBW_channel);
    set(hObject,'Value',1);  

end


%% Service: callback GUIlb_band object
function lb_band_callback(hObject,handles)
    ...
    % Get the correct channel bandwidth name
    sScen.BANDWIDTH = sLTE_stand.cvBW_channel{iVal};
